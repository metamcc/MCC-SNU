...
class Proj_Date_emp_id_PactRate():
    def __init__(self):
        self.inputDataPath = rootPath + 'Data/RawData'
        self.inputDataPath1 = rootPath + 'Data/Prj_List.csv'
        self.outputDataPath = rootPath + 'Data/Variables/Whole/ProjAnalysis/Proj_Date_emp_id_PactRate.csv'
    ...
    def make_Proj_Date_Dict(self):
        ...
        for index,row in self.mainDatadf.iterrows():
            ...
            retDict[row.prj_id][row.dact_date].append([row.emp_id, row.pact_rate])
        ...
        for key in sorted(self.proj_Date_Dict.keys()):
            ...
            for idx,key1 in enumerate(sorted(self.proj_Date_Dict[key].keys())):
                ...
                while i < (len(self.proj_Date_Dict[key][key1])-1):
                    f.write('%s;%s'% (self.proj_Date_Dict[key][key1][i][0],self.proj_Date_Dict[key][key1][i][1]/100))
                    i += 1
                if idx != (lenData-1):
                    f.write('%s;%s&'% (self.proj_Date_Dict[key][key1][i][0],self.proj_Date_Dict[key][key1][i][1]/100))
                else:
